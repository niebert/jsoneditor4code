...
function check_exportvar(pkg) {
	if (!pkg.hasOwnProperty("exportvar")) {
		pkg.exportvar = pkg.name;
	}
}

function getLibs4readme(pkg) {
	var vExportVar = pkg.exportvar || pkg.name;
...
function getURL4WWW(pkg) {
	...
	if (!pkg.hasOwnProperty('gituser')) {
		pkg.gituser = "unknown_gituser";
		console.error("WARING: add 'gituser' as attribute to package.json");
	};
...
function get_zip_url(pkg) {
	var vURL = "https://www.github.com/" + pkg.gituser + "/" + pkg.exportvar + "/archive/master.zip";
	...
	if (giturl) {
		if (giturl.indexOf("gitlab")>0) {
			vURL = "https://www.gitlab.com/" + pkg.gituser + "/-/archive/master/" + pkg.name +  "-master.zip"
		}
	}
	return vURL;
}
